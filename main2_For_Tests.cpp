...
int main(int argc, const char**argv) {
    ...
    std::string randTest("tests/in");
    std::string dotIn(".txt");
    std::string result("outRes");
    FILE *rfile;

    int counter = 0;
    for(int i=1; i < 16; i++) {
        rfile_string = randTest + std::to_string(i) + dotIn;
        wfile_string = result + std::to_string(i) + dotIn;
...
static errorType OnGetAllUnlabeledSegments(void* DS, const char* const command) {
    ...
    fprintf(wfile, "--End of segments list--\n");
